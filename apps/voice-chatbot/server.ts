...
const storage = multer.diskStorage({
    ...
    filename: function (_req: any, file: { fieldname: string; originalname: any; }, cb: (arg0: null, arg1: string) => void) {
...
const fileFilter = (_req: any, file: { mimetype: string; }, cb: (arg0: Error | null, arg1: boolean) => void) => {
...
app.post('/upload', upload.single('audio'), async (_req: any, res: { status: (arg0: number) => { (): any; new(): any; send: { (arg0: string): any; new(): any; }; json: { (arg0: { transcription: any; }): void; new(): any; }; }; }) => {
    const command = `whisper ./`+fileName+` --model small`;

    exec(command, (error: { message: string; }, _stdout: any, _stderr: any) => {
        if (error) {
            console.error('Error executing whisper command:', error);
            return res.status(500).send('Error processing audio' + error.message);
        }

        exec(`cat ./` + fieldname + `.txt && rm ` + fieldname + `*`, (error: { message: string; }, stdout: any, _stderr: any) => {
            if (error) {
...
app.post('/convertToSpeech', async (req: { body: { texts: any; }; }, res: { status: (arg0: number) => { (): any; new(): any; json: { (arg0: { error: string; }): void; new(): any; }; }; setHeader: (arg0: string, arg1: string) => void; send: (arg0: Buffer) => void; }) => {
    try {
